    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-black/50 backdrop-blur-md border-b border-brand-primary/10">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        <button onClick={() => onNavigate('home')} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
          ...
          />
          <span className="font-display font-bold text-xl tracking-tighter uppercase text-brand-primary/90">
        ...
        <div className="hidden md:flex items-center gap-8 text-[10px] font-bold tracking-[0.2em] uppercase">
          ...
          <button className="px-6 py-2 rounded-full gradient-border hover:scale-105 transition-all duration-300 font-bold">
      ...
      {isOpen && (
        ...
        >
          ...
          <button className="w-full py-3 rounded-full bg-brand-primary text-black font-bold">
            Connect
          </button>
        </motion.div>
      )}
